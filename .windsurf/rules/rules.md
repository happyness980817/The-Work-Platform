...
- 사이드바 채팅방 목록 선택/전환, 온라인 상담사 목록/상태 실시간 반영
...
- **메시지**: 소속 세션, 발신자(client/counselor/ai), 텍스트, 전송 시각, 내담자 노출 여부
- **상담사 상태**: 상담사 id, 상태(online/away/offline), 마지막 업데이트 시각

## UI 레이아웃

- **시작화면(대기실)**: 온라인 상담사 목록/상태, 빠른 시작/방 참여 진입점
- **앱 본체**: 좌측 사이드바(채팅방 목록/선택) + 메인 채팅 영역
- **상담사 화면**: 메인 채팅 + 우측 AI 보조 패널 (2열 구조)
- 상담사 전용 기능(AI 패널/히스토리)은 상담사에게만 표시
- Facilitator 아바타 URL(`features/users/data/facilitators.ts`)은 여러 컴포넌트에서 재사용
- About 페이지: 언어별(ko/en) 마크다운 편집 지원, `i18n.language` 기반 전환
...
- **포함**: 로그인/회원가입, 시작화면(온라인 상담사), 사이드바 채팅방 선택/전환, 실시간 채팅, AI 초안 생성/수정/전송
- **제외**: 결제, 관리자 대시보드, 복잡한 권한 시스템, AI 자동 응답, 장기 치료 계획, 공식 진단, 다자간 회의
