...
> # ⚙️ Phase 4 와 동시에 진행 (워크시트 백엔드 연결)
>
> 본 문서는 **워크시트 UI**를 다룹니다. 백엔드(저장/조회) 연결은 **Phase 4**에서 [`chat_app_plan.md`](./chat_app_plan.md) B3와 함께 진행됩니다.
>
> | Phase | 본 문서 관련 작업 |
> |---|---|
> | 4 | `WorksheetOverlay.handleSaveOrComplete` TODO 자리 → 백엔드 mutation 연결 (client `createWorksheet` / facilitator는 `getLatestWorksheet`로 받은 prop 렌더) |
> | 7 | facilitator의 "코파일럿 전송" 흐름 — `copilot_implementation_plan.md` Step 8 참조 |
>
> UI 뼈대 자체는 이미 구현 완료 (오버레이, 선택/작성/감정 화면, 페이지네이션, 토스트, 토글 버튼 등).

`chat-session-page.tsx`에 추가되는 신규 프론트엔드 기능들 (DM 페이지는 영향 없음).

> **사용자 흐름/엣지 케이스/데이터 휘발 정책**: 워크시트 작성/조회/코파일럿 전송 흐름과 결정 사항은 [`chat_decisions_and_flows.md`](./chat_decisions_and_flows.md) 참조. 본 plan은 UI 구조/컴포넌트 계약에 집중.
...
> **퍼실리테이터 화면**에서는 각 워크시트 Item의 버튼 라벨이 **"사용" 대신 "조회"**로 표시됩니다 (이전 계획의 "내용 확인하기" / "코파일럿으로 전송" 추가 버튼은 폐기 — "조회" 하나로 단일화). 클릭 시 facilitator용 작성 화면(읽기 전용 + 마지막 페이지에 "코파일럿 전송" 버튼) 으로 진입.
> 감정 목록은 작성하는 워크시트가 아니라 레퍼런스이므로 role 분기 없이 동일 UI.
...
**Textarea (클라이언트 한정)**: shadcn `Textarea`. 충분히 큰 높이 (예: `min-h-[280px]`). resize-y 정도 허용.

**퍼실리테이터 작성 화면 = 읽기 전용**:

- `<Textarea />` 대신 **plain text 영역**으로 내담자가 전송한 답변을 표시 (`whitespace-pre-line`으로 줄바꿈 보존).
- 답변이 없으면 placeholder "(내담자가 아직 작성하지 않았습니다)" 같은 muted 텍스트 표시.
- 안내문/예시는 클라이언트 화면과 동일하게 노출하여 facilitator도 어떤 prompt에 대한 답인지 즉시 파악 가능.
...
| 2      | 원한다 (I want …)            | "'I want *** (나는 ***를 원한다)'라는 생각이 떠오르면 적습니다.<br>만약 잘 떠오르지 않으면, '이 상황이나 사람이 어떻게 바뀌면 완벽해질까?'를 떠올려 보세요. 상황이 완벽하려면 무엇이 필요한가요?<br>여기서는 '신처럼' 완벽을 설계해도 됩니다.<br>예: '무슨 일이 있어도 늘 제시간에 나타나길 원한다', '그녀가 항상 무엇을 하는지 정확히 알고 싶다'.<br>거의 다 채웠을 때, 스스로에게 묻습니다: '내가 진짜 원하는 걸 썼나?' 아니라면 맨 아래에 진짜 원하는 것을 적습니다."            |
...
### 2.8 마지막 페이지 — 저장/전송 버튼 (TODO)

| Role        | 버튼 라벨            | 클릭 시 동작                                                                       |
| ----------- | -------------------- | ---------------------------------------------------------------------------------- |
| client      | "저장" (또는 "전송") | 작성한 6 페이지 답변을 JSON으로 mutation 호출 → DB 저장                            |
| facilitator | "코파일럿 전송"      | 저장된 worksheet `data` (rich JSON)를 `/api/ai/generate`에 context로 첨부하여 호출 |

- **이번 단계에서는 클릭 시 동작 = TODO 주석**. 백엔드 단계에서 연결.

```ts
const handleSaveOrComplete = async () => {
  // TODO(backend):
  // - client: createWorksheet mutation 호출 (data: jsonb 형태로 저장)
  // - facilitator: /api/ai/generate POST { worksheet: data }
  handleClose();
};
```
...
`chat_app_plan.md` / `copilot_implementation_plan.md` 진행 시 함께 반영해야 할 사항:
...
CREATE TABLE worksheets (
  ...
  session_room_id bigint NOT NULL REFERENCES session_rooms(session_room_id) ON DELETE CASCADE,
  client_id uuid NOT NULL REFERENCES profiles(profile_id) ON DELETE CASCADE,
  type text NOT NULL,        -- 'judge-your-neighbor' | 'when-story-hard'
  data jsonb NOT NULL,       -- 풍부형 payload (아래)
  created_at timestamp NOT NULL DEFAULT now()
);
```

> 감정 목록은 작성 X, 저장 대상 아님.

### `data` payload 형식 — 풍부형 (rich) JSON 권장
...
```json
{
  ...
  "title": "이웃 판단하기 양식",
  "pages": [
    { "index": 1, "subtitle": "1. 누가/왜 화나게 하나요", "answer": "..." },
    { "index": 2, "subtitle": "2. 어떻게 바뀌길 원하나요", "answer": "..." },
    { "index": 3, "subtitle": "3. 어떻게 해야 하나요", "answer": "..." },
    { "index": 4, "subtitle": "4. 무엇이 필요한가요", "answer": "..." },
    {
      "index": 5,
      "subtitle": "5. 그 사람을 어떻게 생각하나요",
      "answer": "..."
    },
    { "index": 6, "subtitle": "6. 두 번 다시는", "answer": "..." }
  ]
}
```
...
- 작성 당시 subtitle/title snapshot 보존 → constants 추후 변경되어도 과거 워크시트 컨텍스트 유지 (fine-tuning 데이터셋 안정성)
- 코파일럿 전송 시 그대로 직렬화 → AI가 풀 컨텍스트 이해
- 단순 배열 (`string[]`) 대비 저장 비용 차이 미미 (jsonb 압축)

### mutations / queries

- **새 mutation** `createWorksheet(client, { sessionRoomId, type, data })` — 클라이언트가 작성 완료 시 호출. `client_id`는 서버에서 `auth.uid()`로 결정
- **새 query** `getLatestWorksheet(client, { sessionRoomId, type })` — facilitator의 "조회" 진입 시 호출 (이 세션의 가장 최근 worksheet 가져오기)

### RLS

- SELECT: 본인이 `client_id`이거나, 같은 세션의 `session_rooms.facilitator_id`인 경우만
- INSERT: `client_id = auth.uid()` (본인만)
- UPDATE/DELETE: 일단 막아도 됨 (작성 후 수정 X)

### 퍼실리테이터 "코파일럿 전송"

- `data` jsonb 그대로 `/api/ai/generate`의 input 컨텍스트로 추가
- `copilot_implementation_plan.md`의 `buildInput`/conversation 흐름 보강 필요
...
8. **퍼실리테이터**: 선택 화면 버튼은 `조회` 하나로 유지. 마지막 페이지의 `코파일럿 전송` 동작은 Phase 7에서 연결
